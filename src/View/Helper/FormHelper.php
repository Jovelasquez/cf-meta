...
namespace CustomFields\View\Helper;

use CustomFields\Utility\Matching;
use CustomFields\View\FlexibleStringTemplateTrait;
 ...
 *
 * @property \CustomFields\View\Helper\HtmlHelper $Html
...
class FormHelper extends \Cake\View\Helper\FormHelper {
    ...
    protected $_defaultConfig = [
        ...
        'templates' => [
            ...
            'search' => '<input type="{{type}}" name="{{name}}" class="form-control{{attrs.class}}" {{attrs}} />',
            'tags' => '<{{tag}} class="{{attrs.class}}">{{content}}</{{tag}}>',
            'tagStart' => '<{{tag}}{{attrs}}>',
            'tagEnd' => '</{{tag}}>',
        ],
        'buttons' => [
    ...
    protected $_defaultWidgets = [
        ...
        'fancyFile' => ['CustomFields\View\Widget\FancyFileWidget', 'file', 'button', 'basic'],
        ...
        'inlineRadioNestingLabel' => ['CustomFields\View\Widget\InlineRadioNestingLabelWidget'],
        'inlineRadio' => ['CustomFields\View\Widget\InlineRadioWidget', 'inlineRadioNestingLabel'],
        'select' => ['Cake\View\Widget\SelectBoxWidget'],
        'selectColumn' => ['CustomFields\View\Widget\ColumnSelectBoxWidget'],
        'textarea' => ['Cake\View\Widget\TextareaWidget'],
        'datetime' => ['CustomFields\View\Widget\DateTimeWidget', 'selectColumn'],
        'search' => ['CustomFields\View\Widget\SearchWidget', 'basic'],
    ];
     ...
     * @return html
     */
    public function renderCustomFields($fields)
    {
        if($fields == null){
            return false;
        }

        return $this->_buildForm($fields);
    }
     ...
     * 
     */
    protected function _getCustomTag($element = null)
    {
        if(in_array($element['field_format'],['text','select','search'])){

        }

        return self::control($element['field_name'], $element['attributes']);       
    }
     ...
     */
    protected function _buildForm($elements, $parent = null){
        $structure = '';

        foreach($elements as $index => $element) {
            if($element['parent'] == $parent) {
                if($element['field_format'] === 'fieldset'){
                    
                    $structure .= $this->formatTemplate('tagStart', [
                        'tag' => $element['field_format'],
                        'attrs' => $this->templater()->formatAttributes($element['attributes']),
                    ]) . $this->formatTemplate('legend', [
                        'text' => $element['name']
                    ]);

                }else{
                    
                    $structure .= $this->formatTemplate('tags', [
                        'tag' => 'div',
                        'attrs' => $this->templater()->formatAttributes([
                            'class' => $element['columns']
                        ]),
                        'content' => $this->_getCustomTag($element)
                    ]);
                }

                $structure .= self::_buildForm($elements, $element['id']);
                if($element['field_format'] === 'fieldset'){
                    
                    if(isset($element['function']) && $element['function']['action'] == "clone"){
                        $structure .= $this->formatTemplate('tags', [
                            'tag' => 'div',
                            'attrs' => $this->templater()->formatAttributes([
                                'class' => 'col-md-12'
                            ]),
                            'content' => $this->formatTemplate('button', [
                                'text' => 'Agregar',
                                'attrs' => $this->templater()->formatAttributes([
                                    'class' => 'btn btn-success btn-sm clone-section',
                                    'data-action' => 'copy',
                                ]),
                            ])
                        ]);
                    ...
                    $structure .= $this->formatTemplate('tagEnd', [
                        'tag' => $element['field_format']
                    ]);
                }
            }
